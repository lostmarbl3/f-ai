...
interface SoloLoginProps {
    onLoginSuccess: (authState: AuthState) => void;
    onBack: () => void;
}

const SoloLogin: React.FC<SoloLoginProps> = ({ onLoginSuccess, onBack }) => {
    ...
    const handleLogin = async () => {
        ...
        setAuthError(null);
        try {
            if (!await authService.isSoloUserRegistered()) {
                if (window.confirm("No passkey found for your profile. Would you like to create one now?")) {
                    await authService.registerSoloUserPasskey();
                } else {
                    setIsAuthenticating(false); return;
                }
            }
            const success = await authService.authenticateSoloUser();
            if (success) {
                onLoginSuccess(authService.loginSolo());
            } else {
                throw new Error("Authentication failed or was cancelled.");
            }
        } catch (err: any) {
            console.error("Solo user authentication error:", err);
    ...
    const handleResetPasskey = async () => {
        if (window.confirm("Are you sure you want to reset your passkey? This will create a new one for this device.")) {
            setIsAuthenticating(true);
            setAuthError(null);
            try {
                await authService.resetAndRegisterSoloUserPasskey();
                ...
                if (success) {
                 ...
                 console.error("Reset and login error for solo user:", err);
    ...
    return (
        <div className="min-h-screen bg-slate-100 dark:bg-slate-900 flex items-center justify-center p-4">
            <div className="max-w-md w-full">
                <Card>
                    <h2 className="text-center text-3xl font-bold text-slate-900 dark:text-white mb-2">Gym Rat Login</h2>
